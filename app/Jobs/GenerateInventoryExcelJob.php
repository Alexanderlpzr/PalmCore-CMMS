...
class GenerateInventoryExcelJob implements ShouldQueue
{
    use Queueable;
    ...
    {
        $path = $this->tenantId.'/'.ExcelReportType::Inventory->filename();
        $export = new InventoryExcelExport($this->tenantId);
        $export->store($path);
        ...
        if ($user) {
            Notification::make()
                ->title('Reporte '.ExcelReportType::Inventory->label().' (Excel) listo')

...
class GenerateReliabilityExcelJob implements ShouldQueue
{
    use Queueable;
    ...
    {
        $path = $this->tenantId.'/'.ExcelReportType::Reliability->filename();
        $export = new ReliabilityExcelExport($this->tenantId);
        $export->store($path);
        ...
        if ($user) {
            Notification::make()
                ->title('Reporte '.ExcelReportType::Reliability->label().' (Excel) listo')

<template>
    <div class="p-5 lg:p-8 max-w-5xl mx-auto">
        ...
        <div class="mb-6">
            ...
            <p v-if="!loading" class="text-sm text-gray-400 mt-0.5">{{ warehouses.length }} almacén{{ warehouses.length !== 1 ? 'es' : '' }}</p>
        </div>
        ...
        <div v-else-if="warehouses.length" class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            >
                <div class="p-5">
                    ...
                    <div class="flex items-start justify-between gap-2 mb-1">
                        <div class="flex items-center gap-2">
                            ...
                            <div>
                                ...
                                <span class="font-mono text-[10px] text-gray-400">{{ wh.code }}</span>
                            </div>
                        </div>
                        <span v-if="!wh.is_active" class="text-[10px] font-semibold bg-gray-100 text-gray-500 px-2 py-0.5 rounded-full shrink-0">Inactivo</span>
                        <span v-else-if="wh.low_stock_count > 0" class="text-[10px] font-bold bg-red-100 text-red-600 px-2 py-0.5 rounded-full shrink-0 flex items-center gap-1">
                    ...
                    <!-- Location -->
                    <div v-if="wh.location" class="flex items-center gap-1.5 text-xs text-gray-400 mt-2 mb-4">
                    ...
                    <div class="grid grid-cols-3 gap-2">
                        <div class="bg-gray-50 rounded-xl p-3 text-center">
                            ...
                            <p class="text-[10px] text-gray-400 mt-0.5">repuestos</p>
                        </div>
                        <div class="bg-gray-50 rounded-xl p-3 text-center">
                            <p class="text-sm font-bold text-gray-900 leading-tight">{{ formatCurrencyShort(wh.total_inventory_value) }}</p>
                            <p class="text-[10px] text-gray-400 mt-0.5">valor est.</p>
                        </div>
                        <div class="rounded-xl p-3 text-center" :class="wh.low_stock_count > 0 ? 'bg-red-50' : 'bg-gray-50'">
                            <p class="text-lg font-bold" :class="wh.low_stock_count > 0 ? 'text-red-600' : 'text-gray-400'">{{ wh.low_stock_count }}</p>
                            <p class="text-[10px] mt-0.5" :class="wh.low_stock_count > 0 ? 'text-red-400' : 'text-gray-400'">bajo mín.</p>
                        </div>
                    </div>

                    <!-- Description -->
                    <p v-if="wh.description" class="text-xs text-gray-400 mt-3 leading-relaxed line-clamp-2">{{ wh.description }}</p>
                </div>
            </div>
        </div>
        ...
        <div v-else class="flex flex-col items-center justify-center py-20 text-center">
            ...
            <p class="text-sm font-medium text-gray-700">Sin almacenes registrados</p>
        </div>
                    ...
                    <div class="relative z-10 w-full sm:max-w-xl bg-white rounded-t-3xl sm:rounded-2xl shadow-2xl max-h-[85vh] flex flex-col">
                        ...
                        <div class="flex-none px-5 py-4 border-b border-gray-100 flex items-start justify-between">
                            <div>
                                ...
                                <p v-if="selectedWarehouse.location" class="text-xs text-gray-400 mt-0.5 flex items-center gap-1">
                            ...
                            <button @click="closeDrawer" class="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-gray-100 text-gray-400 transition-colors shrink-0">
                        ...
                        <div v-else class="flex-1 overflow-y-auto">
                            <div v-if="!drawerStock.length" class="p-8 text-center">
                                <p class="text-sm text-gray-400">Sin repuestos en este almacén</p>
                            </div>
                            <div v-else>
                                    ...
                                    >
                                        <div class="flex-1 min-w-0">
                                            <div class="flex items-center gap-1.5 mb-0.5">
                                                <span class="font-mono text-[10px] text-gray-400">{{ item.spare_part?.code }}</span>
                                                <span v-if="item.spare_part?.is_below_minimum" class="text-[10px] font-semibold text-red-600 bg-red-100 px-1.5 py-0.5 rounded-full">Bajo mín.</span>
                                                <span v-if="item.bin_location" class="text-[10px] text-gray-400 bg-gray-100 px-1.5 py-0.5 rounded-full">{{ item.bin_location }}</span>
                                            </div>
                                        ...
                                        <div class="text-right shrink-0">
                                            ...
                                            <p class="text-[10px] text-gray-400">{{ item.spare_part?.unit }}</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="flex-none px-5 py-4 border-t border-gray-100 flex items-center justify-between">
                            <p class="text-xs text-gray-400">{{ drawerStock.length }} repuestos</p>
